...
|--------|------|-------------------|
| **W1** | [screenshots/adk_web_W1.png](screenshots/adk_web_W1.png) | A2A `transfer_to_agent("search_agent")` → kết quả tìm kiếm multi-agent orchestration |
| **W2** | [screenshots/adk_web_W2.png](screenshots/adk_web_W2.png) | MCP `search_documents("MCP")` trả về tài liệu VinUni AI Curriculum |
| **W5** | [screenshots/adk_web_W5.png](screenshots/adk_web_W5.png) | Prompt `DROP TABLE agent_metrics` gửi qua ADK Web; governance deny verify qua notebook Module 5: `DROP TABLE → deny: Chỉ cho phép SELECT (read-only)` |
...
> **Lưu ý quota Gemini:** Free tier giới hạn ~20 request/ngày/model. Nếu gặp `429 RESOURCE_EXHAUSTED` trên ADK Web, đợi reset quota hoặc dùng session đã chạy thành công trước đó (như W1 session `790066aa-...`).
